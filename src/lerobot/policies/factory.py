...
def make_policy(
    ...
    if cfg.pretrained_path and not cfg.use_peft:
        ...
        if cfg.type == "pi05":
            ...
            for prompt_module, num_tokens, config_field in (
                ("model.vlm_prompt_tokens", cfg.num_vlm_prompt_tokens, "num_vlm_prompt_tokens"),
                ("model.prompt_tokens", cfg.num_prompt_tokens, "num_prompt_tokens"),
            ):
                if num_tokens > 0 and not any(
                    prompt_module == module_name or prompt_module.endswith(f".{module_name}")
                    for module_name in saved_modules
                ):
                    raise ValueError(
                        f"This PI05 PEFT adapter lacks saved learned prompt tokens for {prompt_module}. "
                        "Start from a base checkpoint for new prompt training, or load a matching "
                        f"prompt-enabled adapter. Set {config_field}=0 only for an explicit legacy "
                        "adapter ablation."
                    )
        ...
        if cfg.type == "pi05":
            # Adapter loading bypasses PI05Policy.wrap_with_peft and may enable backbone adapters.
            policy.get_base_model().model._freeze_backbones()
