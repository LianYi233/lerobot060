...
how much the next VLM and action-side AdamW updates would change the predicted flow velocity, then
scales only the action-side optimizer step to enforce a long-run functional-drift budget. It replaces
the action-gradient clipping hook when enabled:
...
| `cabo_action_drift_ratio`       | `0.1`    | Maximum action/VLM RMS functional-drift ratio               |
| `cabo_probe_interval`           | `8`      | Steps between CABO flow-velocity probes                     |
| `cabo_probe_batch_size`         | `1`      | Per-rank samples used by each CABO probe                    |
